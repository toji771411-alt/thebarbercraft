...
export default function AuthModal({ open, onClose, defaultTab = 'login' }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const u = await login(loginEmail, loginPw);
      onClose(); reset();
          ...
          {tab === 'login' ? (
            <div className="space-y-4">
              ...
              <div className="relative my-6">
                ...
                <div className="relative flex justify-center text-xs uppercase"><span className="bg-white px-2 text-gray-400 tracking-widest">Or continue with</span></div>
              </div>

              <button onClick={handleGoogleLogin} disabled={loading}
                className="w-full flex items-center justify-center gap-3 py-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors text-sm font-semibold">
